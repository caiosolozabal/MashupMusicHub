...
```bash
git add .
git commit -m "Descrição da mudança (ex: atualizado Audio Experience)"
git push
```
...
Para que as fotos reais apareçam nos cards de locação, você deve criar as seguintes pastas dentro de `public/` e colocar arquivos `.jpg` com os nomes indicados:
