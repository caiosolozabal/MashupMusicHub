  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [eventSnap, listSnap, subSnap] = await Promise.all([
          getDoc(doc(db, 'guest_events', slugData.eventId)),
          getDoc(doc(db, 'guest_events', slugData.eventId, 'lists', slugData.listId)),
          getDoc(doc(db, 'guest_submissions', submissionId))
        ]);

        if (eventSnap.exists() && listSnap.exists() && subSnap.exists()) {
          setData({
            event: { id: eventSnap.id, ...eventSnap.data() } as GuestEvent,
            list: { id: listSnap.id, ...listSnap.data() } as GuestList,
